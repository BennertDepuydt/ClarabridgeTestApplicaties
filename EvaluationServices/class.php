...
foreach($files as $file)
{
    ...
    for($i=0;$i< count($data); $i++){
        ...
        $test = $result->getIndexResults($i);
        $results2test[$i]["query"] = $data[$i]["query"];
        $results2test[$i]["services"][substr($file,0,-5)]["intent"] = $test["intent"];
        $results2test[$i]["services"][substr($file,0,-5)]["score"] = $test["score"];
    }
    $result->toJson($file);
